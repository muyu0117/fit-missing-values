...
def EMalgorithmFP(epsi,FP,nu,tol):
    ...
    # For details, see here: https://www.arpm.co/lab/redirect.php?permalink=EMFPMissVal
    ...
    while any(Error>tol):
        ...
        for t in range(t_):
            ...
            w[0, t] = (nu+A)/(nu+(eps[a, [t]]-m[a, [j-1]]).T.dot(invs2.dot(eps[a, [t]]-m[a, [j-1]])))

            if na:
                #fill entries
                # eps[na,t]=(m[na,[j-1]]+s2[np.ix_(na,a,[j-1])].squeeze()@invs2@(eps[a,[t]]-m[a,[j-1]])).flatten()
                eps[na, t] = (m[na, [j-1]] + s2[np.ix_(na, a, [j-1])].squeeze().dot(invs2).dot(eps[a, [t]]-m[a, [j-1]])).flatten()

                #fill buffer
                # gamma[t][np.ix_(na,na)]=s2[np.ix_(na,na,[j-1])].squeeze()-s2[np.ix_(na,a,[j-1])].squeeze()@invs2@s2[np.ix_(a,na,[j-1])].squeeze()
                gamma[t][np.ix_(na,na)]=s2[np.ix_(na,na,[j-1])].squeeze()-s2[np.ix_(na,a,[j-1])].squeeze().dot(invs2).dot(s2[np.ix_(a,na,[j-1])].squeeze())

        #step[1:] update output
        # new_m=(eps@(FP*w).T)/npsum(FP*w)
        new_m=(eps.dot((FP*w).T))/npsum(FP*w)        # check if the order is right
        m = r_['-1',m,new_m]
        ...
        new_s2= (eps-tile(m[:,[j]],(1,t_))).dot(diagflat(FP*w)).dot((eps-tile(m[:,[j]],(1,t_))).T)+npsum(gamma_p,2)
