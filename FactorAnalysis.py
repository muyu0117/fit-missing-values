...
def FactorAnalysis(c2,d,k_):
    ...
    beta = np.real(e_ord[:n_, :k_].dot(np.diagflat(maximum(lam, eps1))))
    c = c2

    for j in range(NmaxIter):
        #1. Conditional PC
        # a = c-eye(n_)+np.diagflat(diag(beta@beta.T))
        a = c-eye(n_)+np.diagflat(diag(beta.dot(beta.T)))
        if CONDITIONAL==1:
        ...
        beta_new = E.dot(np.diagflat(maximum(lam, eps1)))
        #3. Rows length
        ...
        c = beta_new.dot(beta_new.T)+eye(n_, n_)-diag(diag(beta_new.dot(beta_new.T)))
