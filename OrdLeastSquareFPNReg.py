...
def OrdLeastSquareFPNReg(X, Z, p, smartinverse=0):
    ...
    # For details, see here: https://www.arpm.co/lab/redirect.php?permalink=OLSFPEstimRegLoad
    ## code
    n_,t_ = X.shape
    k_ = Z.shape[0]
    ...
    if smartinverse == 0:
        ...
        beta_OLSFP = s_XZ.dot(SmartInverse(s_ZZ))

    # alpha_OLSFP = m_XZ[:n_] - beta_OLSFP@m_XZ[n_:n_+k_]
    alpha_OLSFP = m_XZ[:n_] - beta_OLSFP.dot(m_XZ[n_:n_+k_])

    # U = X - tile(alpha_OLSFP, (1,t_)) - beta_OLSFP@Z
    U = X - tile(alpha_OLSFP, (1, t_)) - beta_OLSFP.dot(Z)
