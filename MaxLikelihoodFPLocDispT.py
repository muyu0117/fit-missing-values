...
def MaxLikelihoodFPLocDispT(epsi,p,nu,threshold,last=0,smartinverse=0,maxiter=10**5):
    ...
    # For details, see here: https://www.arpm.co/lab/redirect.php?permalink=EB_MaxLikelihoodFPFun
    ...
    while npsum(error>threshold) >= 1 and k<maxiter:
        ...
        mu_MLFP = r_['-1', mu_MLFP, (npsum(tile(p*w, (i_, 1))*epsi, 1)/(p.dot(w.T)))[..., np.newaxis]]
        epsi_c = epsi-tile(mu_MLFP[:,[k]],(1,t_))
        # sigma2_MLFP = r_['-1',sigma2_MLFP, (epsi_c@np.diagflat(p*w)@epsi_c.T)[...,np.newaxis]]
        sigma2_MLFP = r_['-1', sigma2_MLFP, (epsi_c.dot(np.diagflat(p*w).dot(epsi_c.T)))[..., np.newaxis]]
