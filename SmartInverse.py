...
def SmartInverse(sig2,tol=EPS):
    ...
    if rcond<tol:
        n_=sig2.shape[0]
        k_=int(max(1,round(0.1*n_))) #We empirically choose a number of factors equal to about 10% of the dimension of sig2

        # Step 1. Correlation matrix
        sig2_vol_inv=diagflat(diag(sig2)**(-1/2))
        # rho2=sig2_vol_inv@sig2@sig2_vol_inv
        rho2 = sig2_vol_inv.dot(sig2).dot(sig2_vol_inv)


        #Step 2. Factor analysis
        _, beta, _, _, _ =  FactorAnalysis(rho2,np.array([[0]]),k_)
        # delta2=diag(eye((n_))-beta@beta.T) #Residual vector
        delta2 = diag(eye((n_))-beta.dot(beta.T))
        ...
        rho2_inv = omega2 - omega2.dot(beta).dot(solve(beta.T.dot(omega2).dot(beta)+eye((k_)), beta.T)).dot(omega2)

        #Step 4. (Approximated) inverse of sig2
        # sig2_inv=sig2_vol_inv@rho2_inv@sig2_vol_inv
        sig2_inv = sig2_vol_inv.dot(rho2_inv).dot(sig2_vol_inv)
    else:
        sig2_inv=inv(sig2)
